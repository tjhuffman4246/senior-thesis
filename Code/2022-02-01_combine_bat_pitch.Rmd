...
performance %>% 
  mutate(pa_bf = if_else(is.na(pa_total), bf_total, pa_total)) %>% 
  select(-c(pa_total, bf_total)) %>% 
  write_csv('../Data/Clean/performance_min20.csv')
...
player_mvmt_unwtd %>% 
  write_csv('../Data/Clean/mvmt_obsv_unwtd.csv')

player_mvmt %>% 
  write_csv('../Data/Clean/mvmt_obsv.csv') # indicating observed movement
