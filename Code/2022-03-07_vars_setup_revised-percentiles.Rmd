---
title: 'Data Cleaning: Revised Percentiles'
author: "Tate Huffman"
date: "3/7/2022"
output: pdf_document
---
...
bat_performance <- bat %>%
  ...
  # 2/3 percentile this time instead of 1/2
  # < 1/3 percentile is bad, > 2/3 is good, between is meh
  mutate(ops_threshold_high = quantile(OPS, probs = 2/3, na.rm = TRUE),
         ops_threshold_low = quantile(OPS, probs = 1/3, na.rm = TRUE),
         signal = case_when(OPS >= ops_threshold_high ~ 1,
                            OPS <= ops_threshold_low ~ -1,
                            TRUE ~ 0)) %>% 
  ungroup() %>% 
...
pitch_performance <- pitch %>%
  ...
  mutate(era_threshold_low = quantile(ERA, probs = 1/3, na.rm = TRUE),
         era_threshold_high = quantile(ERA, probs = 2/3, na.rm = TRUE),
         signal = case_when(ERA <= era_threshold_low ~ 1,
                            ERA >= era_threshold_high ~ -1,
                            TRUE ~ 0)) %>% 
  ungroup() %>% 
  arrange(Name, Year, Level) %>% # makes it easier to get lowest-level signal (our signal)
  group_by(Name, Year) %>% 
...
bat_performance %>% 
  write_csv('../Data/Clean/bat/bat_performance_min-20pa_revised-pct.csv')

pitch_performance %>% 
  write_csv('../Data/Clean/pitch/pitch_performance_min-20bf_revised-pct.csv')
