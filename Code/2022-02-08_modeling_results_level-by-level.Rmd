...
n_years <- 10 # the number of seasons to look at in the data

# Performance filter

performance_filtered <- performance %>% 
  filter(level_low == level_num) %>% 
  group_by(Name) %>% 
...
player_mvmt_filtered %>% 
  select(-cell) %>% 
  write_csv("../Scratch/2022-02-10 Results/Rookie/2022-02-10_result_probs_rookie.csv")
