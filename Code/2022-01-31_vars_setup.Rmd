...
bat_performance <- bat %>%
  ...
  ungroup() %>% 
  filter(pa_total >= 40) %>% # filtering out pitchers, for the most part
  group_by(Year, Level) %>% 
...
pitch_performance <- pitch %>%
  ...
  mutate(promotion = if_else(level_high < lead(level_low, default = 0), 1, promotion)) %>% 
  ungroup() %>% 
  filter(bf_total >= 20) %>% # filtering out position players, for the most part
  group_by(Year, Level) %>% 
...
bat_performance %>% 
  write_csv('../Data/Clean/bat/bat_performance_min_40pa.csv')
