...
bat_performance <- bat %>%
  mutate(Level = case_when(Level == "Rookie" ~ 1,
                           ...
                           Level == "AA" ~ 5,
                           Level == "AAA" ~ 6,
                           Level == "MLB" ~ 7)) %>%  # converts level to scalar for ease of computation
  group_by(Name) %>% 
  mutate(yr_unique = cumsum(!duplicated(Year)), # counter of unique year played
         exit = if_else(yr_unique == max(yr_unique), 1, 0)) %>% # whether the player exited after that year
  ungroup() %>%
  group_by(Name, Year) %>% 
  ...
  mutate(Age = max(Age),
         ...
         level_low = min(Level), # both high and low levels, so we know if next year's level is strictly greater (for promotions)
  ...
  ungroup() %>% 
  arrange(Name, Year, Level) %>% # arranges by level to make it easier to get lowest-level signal (the signal for our purposes)
  group_by(Name, Year) %>% 
  summarize(age = Age,
            pa_total = pa_total,
  ...
  filter(yr_unique <= n_years, # takes only the first ten years of someone's career
         yr_debut >= 1999, # the year after our first year of data, so we know that the first observation is a debut
         yr_debut <= 2009) # so that we have 10 + 1 years of data for everyone, in order to know whether they exit at the end
